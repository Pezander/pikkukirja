...
export async function POST(
  ...
  const overdueOnly = !!body.overdueOnly;
  const now = new Date();

  const [allInvoices, association] = await Promise.all([
  ...
  // Separate paid (skip) vs unpaid
  const paid = allInvoices.filter((inv) => inv.paidAt);
  const unpaid = allInvoices.filter((inv) => !inv.paidAt);
  const toSend = overdueOnly ? unpaid.filter((inv) => inv.dueDate < now) : unpaid;
  ...
  for (const invoice of toSend) {
    ...
    try {
      ...
      await transport.sendMail({
        ...
        subject: `MUISTUTUS: Lasku ${invoice.invoiceNumber} – ${association.name}`,
        html: `
          <p>Hyvä ${invoice.member.name},</p>
          <p>haluamme muistuttaa avoimesta laskusta <strong>${invoice.invoiceNumber}</strong> organisaatiolta <strong>${association.name}</strong>.</p>
          ${isOverdue ? `<p style="color:#c00;font-weight:bold">Lasku on erääntynyt ${fmtDate(invoice.dueDate.toISOString())}.</p>` : ""}
  ...
  return NextResponse.json({ sent, noEmail, failed, skipped: paid.length });
}
